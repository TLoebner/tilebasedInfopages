...
//   if (is_numeric($key1)){
//     $query = "UPDATE rubrikeninhalte SET fk_rubrik='".$value1['rubrik']."', reihe=".$value1['reihe'].", inhalt='".htmlentities($value1['inhalt'])."', link='".$value1['link']."', comment='".htmlentities($value1['comment'])."' WHERE pk=".$key1;
//     $mysqli->query($query);
//   }
// }
foreach ($ergarray as $key1 => $value1){
        ...
        for ($m=1;$m<=5;$m++){
            if ($value1['rubrik'.(string)$m] > 0){
                ...
                if ($res_multicontent->num_rows > 0){
                    $query = "UPDATE help_multicontents set uid='".$uid."', sequence_no=".$value1['sequence'.(string)$m]." WHERE help_subsection_fk=".$value1['rubrik'.(string)$m]." AND help_content_fk=".$key1.";";
                }else{
                    $query = "INSERT INTO help_multicontents  ( uid,sequence_no, help_subsection_fk, help_content_fk ) VALUES ( '".$uid."',".$value1['sequence'.(string)$m]." , ".$value1['rubrik'.(string)$m]." , ".$key1." );";
                }
                $mysqli->query($query);
...
<td valign="top">
<form name="edit_help" method="POST" action="edit_help.php?pk=<?PHP echo $_GET['pk']?>">
<?PHP
    $m=1;
    ...
    echo "<table>";
    echo "<tr><td colspan=\"3\"><a href=\"delete_help.php?pk=".$_GET['pk']."\">Kompletten Eintrag löschen</a></td></tr>";
    echo "<tr><td colspan=\"3\">&nbsp;</td></tr></table><table>";
    for ($m=1;$m<=5;$m++){
        echo "<tr><td><a href=\"delete_helpmulti.php?sec=".(string)$section_fk[$m]."&cont=".(string)$dataTblHelpContents[$_GET['pk']]['pk']."\">Gruppe löschen</a></td><td align=\"center\"><select name=\"in_".(string)$dataTblHelpContents[$_GET['pk']]['pk']."_rubrik".(string)$m."\">\n";
