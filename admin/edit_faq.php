...
//         fk_rubrik=".$value1['rubrik'].", reihe=".$value1['reihe'].", question='".$value1['question']."', answer='".$value1['answer']."', link='".$value1['link']."' WHERE pk=".$key1;
        ...
        $log->WriteLog;
        $dataTblFaqContents[$key1]['question'] = $value1['question'];
        $dataTblFaqContents[$key1]['answer'] = $value1['answer'];
        $dataTblFaqContents[$key1]['link'] = $value1['link'];
        /**
         *
...
?>
<form name="edit_faq" method="POST" action="index.php?action=EditFaq&pk=<?PHP echo $_GET['pk']?>">
<table>
<?PHP
    $m=1;
    ...
    for ($m=1;$m<=5;$m++){
        echo "<tr><td><a href=\"index.php?action=DeleteFaqMultiConts&sec=".(string)$section_fk[$m]."&cont=".(string)$dataTblHelpContents[$_GET['pk']]['pk']."\">Gruppe löschen</a></td><td align=\"center\"><select name=\"in_".(string)$dataTblFaqContents[$_GET['pk']]['pk']."_rubrik".(string)$m."\">\n";
    ...
    echo "<tr><td colspan=\"2\" >&nbsp;&nbsp;&nbsp;<a href=\"index.php?action=DeleteFaq&pk=".$_GET['pk']."\">Kompletten Eintrag löschen</a></td></tr>";
